...
8. **設計判断は実装後の実証で更新する**（L-024・2026-05-24追加）：重要な設定変更・規律設計時は、設計判断時の想定だけで進めず、(a)**既存設定の現状把握**を行ってから判断、(b)実装後に**想定効果と実効効果のズレを必ず突合**し、ズレが大きければ判断を更新する。原則7「事実確認の優先順位」の延長として、設定変更（settings.json／agent定義／Skill）の事前確認と事後検証の両面ゲート。
