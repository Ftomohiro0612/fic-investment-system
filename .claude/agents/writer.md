...
- **Phase 0標準チェック確認（L-029・2026-05-24追加）**：`handoff_researcher_company_to_writer.md` §4.0「Phase 0標準チェック結果」を確認。researcher段階で既存資料のPhase 5観点再検証が完了していることを必須前提。未確認なら writer開始前にFIC確認。詳細は `docs/lessons_3layer_pattern.md` §8。
...
- **FIC意見表出義務（L-027）**：本記事5論点【中計達成可能性／主要先行指標方向感／M&A減損リスク／競合相対評価／為替・原料中期方向】のうち**最低3点**でFIC意見を組み込む。FIC意見ラベル（FIC評価／FIC見立て／会計士視点では）を明示。詳細は [[expression-strength-rules]] §10。
- **会計士視点は"実演"する（L-027の中身定義・2026-06-02）**：看板3ムーブ＝**①利益の質（一時要因を除いた地力）／②利益とキャッシュの乖離／③開示の行間**（[[writing-style]] §7-β）を本文で**最低1〜2点実演**し💡でかみ砕く。「会計士視点で」とラベルを貼るのでなく分析そのもので示す。
- **読者向け文章の地雷（[[writing-style]] §5-α・#20の20項目差し戻し）**：規律・手法・前提のメタ叙述を本文に書かない（横串/fact-safetyは裏で守る）／専門語・固有名詞は初出で言い換えor📘／表に導入＋読み取り／汎用概念は/learn/リンク／周辺は比例配分（空カテゴリを立てない）／主要ドライバーは"なぜ"まで（深さは削らない）／**読者向けにFIC非表示**（自称＝「会計士の視点では」「本記事の試算」等）。ブランド＝**会計士とよむ決算**。
- **FIC独自ドライバー視点組み込み（L-028）**：researcher §8-B（FIC独自ドライバー候補）の4視点（隠れ／分解／実質比重／同業逆算）のうち**最低1点**を記事のドライバー構成に組み込む。判断フロー（並列/優先採用/FIC確認必須）は `docs/independent_driver_lessons.md` §2 末尾参照。
...
- **読者ペルソナ＝投資を始めたばかり〜2・3年目（2026-06-02確定）。入口は「角度の発見」**（[[writing-style]] §7-α）：タイトル・メタ・冒頭は"どの切り口を主役にするか"を先に決める（「なぜ意外／なぜ今効くか」）。**メタディスクリプション必須**、冒頭1文目は専門用語の塊で始めない。煽り・断定・下ネタは禁止（不変）。背景は `docs/strategy_persona_tone_pivot.md`・Issue #14。
