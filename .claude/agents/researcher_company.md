...
- **★業績ドライバー候補は「公式版＋FIC独自版」の両併記必須（L-028・2026-05-24追加）**：pack §1.8の「業績ドライバー候補4テーマ案」は **§8-A 公式ドライバー（会社中計／セグメント区分／会社感応度に直結）** ＋ **§8-B FIC独自ドライバー（4視点：隠れ／分解／実質比重／同業逆算のうち最低1点・根拠付き・該当する全てを列挙）** ＋ **§8-C writer申し送り（採用判断材料）** の3部構成。**researcher は4視点のうち該当する全てを §8-B に列挙し、最終的にどの視点を採用するかは writer の選択裁量**。詳細は `docs/independent_driver_lessons.md`。
