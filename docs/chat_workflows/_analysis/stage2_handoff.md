...
- **★追加評価軸（FIC合意 2026-05-22）：統合メモはv4 packに対し独自価値を出したか**（統合メモは現状維持で走らせ実地評価）：
  - 網羅性の担保（v4 packに含まれない素材を統合メモが補ったか）／章横断の整合チェック（章間の論理整合を統合メモが確保したか）／reviewer点検時の参照価値（reviewerが実際に統合メモを参照したか）／v4 packとの重複度（どの部分が単純コピーか）。
  - 段階4 writer定義への直結：価値あり→統合メモ生成節を維持／重複大→軽量化（要点のみ）or 廃止判断。机上でなく実証データで決める（[[meta-vs-real-work]]）。
- **段階3パイロットで育てる領域**：
...
### 段階3パイロット 実行結果（2026-05-22・writer/reviewerループ完走）
- **ループ完走**：writer→reviewer(R1: blocker0/major3/minor2)→writer(R2上書き)→reviewer(R2: blocker0/major0/minor1)→`handoff_reviewer_to_designer.md`（合格）。上限2周以内。成果物＝`work/company_analysis/285A_kioxia_pilot/`（claude_integrated_memo.md / claude_article.html〔公開15章・57KB〕/ claude_review_notes.md〔R1/R2履歴〕/ handoff 3本）。Sheets更新はパイロットのためスキップ。
- **差し戻しループの堅牢性を実証**：reviewer R1 が EBITDA「約7,641億円」を年度取り違え（764.1＝FY2024）→ writer が原文照合で FY2025＝約1兆1,879億円に補正 → reviewer R2 が原文で再確認し writer 補正を是認。**reviewer の指摘も原文照合で相互検証すべき**（指摘を鵜呑みにしない）。
- **統合メモは独自価値あり（追加評価軸の暫定結論）**：reviewer が実際に全面参照。記事grep＋メモ突合＋extracted_text原文照合の3点照合に有用。とくに§10で会社開示Net D/E・EBITDAとFIC参考計算を併記し差分（スワップレート差まで）を注記＝source-hierarchy運用の深度を記事単体より担保。**段階4 writer定義：統合メモは維持（軽量化不要）。「出所順位の判断・FIC試算と会社開示の差分説明」を残す運用を標準化推奨**。一方、会社開示済みだがpack段階で拾われなかった指標（Net D/E 39%）はメモも取りこぼし＝reviewerの原文照合は依然必須。
- **環境制約（記録）**：本セッション（FleetView/SDK）では `.claude/agents/` のカスタムsubagentが Agentツールから直接起動できず、general-purposeに writer.md/reviewer.md を運用仕様として読み込ませる代理方式で実行。実Claude Code CLIではネイティブ起動可。設計（writer.md/reviewer.md の中身）の検証としては有効。
- **FIC確定事項（事後承認待ち）**：①章7ドライバー＝ASP×bit統合で実質3本／②章8見出し＝「長期財務モデルと成長戦略の検証」／確認3〜7はA採用。
- **新旧比較（成功基準）＝チェックリスト①合格・比較材料②作成済み**：旧版（kioxia-holdings-285a-analysis）をWebFetchし8軸で構造化比較 → **上回る4軸（構成・出所規律・横串・リスク対称性）／同等3軸（分析深度・初心者間口・参照等）／下回る0軸・明確な後退なし**。暫定で成功基準「同等以上」を満たす方向。**残るはFIC目視の最終判定**（特に：入口タイトル強度／用途別利益の推定回避vs情報量／参照9→6件／反証閾値が新版は方向ベース・旧版は期間付き定量で具体＝ここは旧版優位の可能性／章8見出しの収まり）。
- **レビュー教訓ログ新設**：`docs/chat_workflows/_analysis/review_lessons_log.md`（L-001〜L-008＋「教訓昇格判定基準」節）。subagentは永続記憶を持たないため、writerを賢くする＝writerが読む静的資産（Skill/guide/checklist/template/writer.md）の更新。
- **R3（FIC指示の追加対応）**：GPT外部レビュー（独立評価でも新版88-91>旧版80-83）を踏まえ、採用3点を記事に反映＝①FY注釈を冒頭📘に固定（1Q予想≠通期の誤読防止）②章10.2にハイパースケーラーCapEx追加③反証条件を「N四半期連続」観測パターンに精緻化（章2.3＝12.4＝13.3一致）。GPT提案のうちSanDisk・図解は既充足/designer段階4のため対応不要、タイトル中立化はFIC判断で現状維持。
- **R4（FIC指示の文体基調変換）**：FICの違和感（堅すぎる＝常体主体）を実測で裏取り（地の文常体70件超・敬体40・疑問形0）→ writing-style §1/§5の意図（親しみやすさ・敬体例）と乖離と判定。**本文を です・ます基調へ変換**（常体文末ほぼ0・敬体238・軽い疑問形3）。分析深度・数値・構成・R3の3点は不変。
- **教訓の即時反映（C'案＝基準ゲート付き・FIC承認 2026-05-22）**：subagentが「賢くなる」1サイクルを段階3内で実証。即時反映＝L-002 IFRSラベル（factual-handling §3）／L-005 反証パターン（chapter_design_guide 章12/13）／L-006 予想範囲明示（factual-handling §3＋template）／L-008 文体基調=です・ます（writing-style §5）。**§7ゲートはfactual-handling・writing-styleの2点のみ・FIC承認済**。L-007 ハイパースケーラCapExは既存4基準で導出可能のためSkill非反映（記事内のみ・段階4再検討）。判定基準＝「論理的原則 or 両ソース裏取り済み普遍ルール=即時／データ的一般化=段階4」。
- **段階3の到達点**：成功基準①チェックリスト＝合格／②比較＝新版が旧版に対し上回る4軸・同等3軸・下回る0軸（GPT外部評価も新版優位）。**writer/reviewerループ＋教訓の即時反映まで実証**＝Phase 5移行価値（subagentが賢くなる）の実証完了。designer工程（図解・WP反映）は段階4。
