...
- 関連銘柄には、需要側/中間材側/上流側などのサプライチェーン3区分ラベルを必ず入れる。主役企業は、決算で見る具体的なIR開示項目名まで書く。
- `不採用候補` とは別に `要追加検討候補` を置く。本文採用の可能性はあるが、投入パック作成時点で根拠や重要度が足りない企業をここに残す。
- 原油、ナフサ、金利、為替、素材などの上流変数を起点にするテーマでは、上流スプレッド、稼働率、中間材価格、上流セグメント利益を先行指標表に入れる。
- TDBなどの調査比率、市場シェア、要因比率を使う場合は、分母と対象範囲の注釈を入れる。回答比率や値上げ要因比率を、総コスト比率のように読ませない。
- タイトル3案は、`抽象寄り` `検索流入寄り` `バランス型（推奨）` などの性格ラベルと、推奨理由を1行ずつ添える。
- アナリスト・専門家コメント候補は最低1件探す。見つからない場合は `該当なし/採用なし` と理由を明記する。
- 起点イベントは、日本側公式だけでなく、海外側公式または海外主要メディア、主要メディアの並列確認を試す。該当しない国内テーマでは `海外側該当なし` と明記する。
- 冒頭リード2段落ドラフトは空欄にしない。30秒要約の前に置ける文章として、Codex側で必ず仮文を渡す。
