...
1. **新規事業・ニュース解説型**: 企業の新規事業発表、工場建設、提携、投資計画、M&A、政策採択など。`何が発表されたのか → なぜ重要なのか → 事業モデル → 技術/仕組み → 数字で見るインパクト → 企業価値への影響 → 競争環境 → ボトルネック → 反対シナリオ → 投資家が見るべき指標 → まとめ → FAQ → 参照資料`。
2. **マクロ・テーマ記事型**: 金利、為替、半導体市況、原油高、防衛、インバウンド、AI電力需要など。`トレンドの概要と発生要因 → 影響経路 → 恩恵セクター → 主要企業で見るべきポイント → 逆風セクター → ボトルネック → 先行指標 → 3シナリオ → 反対シナリオ → 投資家が見るべきポイント → まとめ → FAQ → 参照資料`。
